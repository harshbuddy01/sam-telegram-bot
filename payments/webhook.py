...
async def handle_cashfree_webhook_get(request: web.Request) -> web.Response:
    return web.json_response({"status": "ok", "service": "Cashfree Webhook Gateway Active"})

async def handle_cashfree_webhook(request: web.Request) -> web.Response:
    bot: Bot = request.app["bot"]
    body_bytes = await request.read()
    
    if not body_bytes:
        return web.json_response({"status": "ok", "message": "empty_ping"})
    
    try:
        payload = json.loads(body_bytes.decode("utf-8"))
    except Exception as e:
        logger.error(f"Failed to parse Cashfree webhook JSON: {e}")
        return web.json_response({"status": "ok", "message": "ignored_non_json"})

    # Cashfree Test Ping / Setup Verification
    if not isinstance(payload, dict) or ("data" not in payload and "order" not in payload and "type" not in payload):
        logger.info("Received Cashfree webhook test ping")
        return web.json_response({"status": "ok", "message": "test_ping_received"})

    event_type = payload.get("type", "")
    data = payload.get("data", {})
    order_data = data.get("order", {}) or payload.get("order", {})
    payment_data = data.get("payment", {}) or payload.get("payment", {})

    order_id = order_data.get("order_id") or payload.get("order_id")
    payment_status = payment_data.get("payment_status") or payload.get("payment_status", "")
    payment_id = str(payment_data.get("cf_payment_id") or payment_data.get("payment_id") or "CF_PAID")

    logger.info(f"Received Cashfree Webhook: event={event_type}, order_id={order_id}, status={payment_status}")

    # Process successful payment
    if payment_status.upper() in ("SUCCESS", "PAID") or event_type in ("PAYMENT_SUCCESS_WEBHOOK", "ORDER_PAID"):
        if not order_id:
            return web.json_response({"status": "ok", "message": "no_order_id"})

        async with AsyncSessionLocal() as session:
            stmt = select(Deposit).where(Deposit.gateway_order_id == str(order_id))
            res = await session.execute(stmt)
            deposit = res.scalar_one_or_none()
            ...
            if not deposit:
                ...
                for p in all_pending:
                    if p.gateway_order_id and (str(order_id) in p.gateway_order_id or p.gateway_order_id in str(order_id)):
                        deposit = p
                        break

            if not deposit:
                logger.info(f"No pending deposit matching Cashfree order_id={order_id}")
                return web.json_response({"status": "ok", "message": "ignored_no_match"})

            if deposit.status in ("APPROVED", "SUCCESS"):
                logger.info(f"Deposit #{deposit.id} already processed. Skipping duplicate webhook.")
                return web.json_response({"status": "ok", "message": "already_processed"})

            deposit, user = await credit_user_deposit_automated(session, deposit.gateway_order_id, payment_id)
            if not deposit or not user:
                return web.json_response({"status": "ok", "message": "credit_failed"})

            logger.info(f"Cashfree deposit #{deposit.id} credited for User {user.telegram_id} (₹{deposit.amount})")
                ...
                if target_var:
                    ...
                    if order and getattr(order, "delivered_content", None):
                        delivery_text = (
                            ...
                            f"{ce(CustomEmojis.ORDERS, '🧾')} <b>Order ID:</b> #{order.id}\n"
                            f"{ce(CustomEmojis.SHOP, '📦')} <b>Product:</b> <b>{prod_title}</b>\n"
                            f"{ce(CustomEmojis.SPARKLE, '✨')} <b>Plan:</b> {target_var.name}\n"
                            f"{ce(CustomEmojis.WALLET, '💰')} <b>Amount Paid:</b> {config.CURRENCY_SYMBOL}{order.amount:.2f}\n"
                            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                            f"{ce(CustomEmojis.KEY, '🔑')} <b>YOUR DELIVERED ACCOUNT / KEYS:</b>\n"
                            f"<pre><code>{order.delivered_content}</code></pre>\n\n"
                            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                            f"{ce(CustomEmojis.WARRANTY, '🛡️')} <i>All purchases are backed by 100% replacement warranty!</i>"
                        )
                        try:
                            from keyboards.user_keyboards import get_order_fulfillment_keyboard
                            await bot.send_message(user.telegram_id, delivery_text, reply_markup=get_order_fulfillment_keyboard(order.id))
                        except Exception as e:
                            logger.error(f"Failed to send auto delivery to user {user.telegram_id}: {e}")

                        from handlers.order import _background_notify
                        import asyncio
                        stock_left = await get_available_stock_count(session, target_var.id)
                        asyncio.create_task(_background_notify(
                            bot, order, prod_title, target_var, user, stock_left, order.amount
                        ))
                        return web.json_response({"status": "ok"})

                    elif is_manual:
                        manual_order = await create_order(
                            session=session,
                            ...
                            customer_input=deposit.customer_input or "Provided via 1-Click Cashfree Buy",
                            quantity=qty
                        )
                        dispatch_time = getattr(target_var, "manual_dispatch_time", "1–2 Hours") or "1–2 Hours"
                        manual_confirm_text = (
                            f"{ce(CustomEmojis.CHECK, '✅')} <b>ORDER PLACED SUCCESSFULLY!</b>\n"
                            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                            f"{ce(CustomEmojis.ORDERS, '🧾')} <b>Order ID:</b> #{manual_order.id}\n"
                            f"{ce(CustomEmojis.SHOP, '📦')} <b>Product:</b> <b>{prod_title}</b>\n"
                            f"{ce(CustomEmojis.SPARKLE, '✨')} <b>Plan:</b> {target_var.name}\n"
            ...
            # Normal Top-up deposit
            deposit_msg = (
                f"{ce(CustomEmojis.SPARKLE, '🎉')} <b>CASHFREE DEPOSIT CONFIRMED & CREDITED!</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"{ce(CustomEmojis.ORDERS, '🧾')} <b>Deposit ID:</b> #{deposit.id}\n"
                f"{ce(CustomEmojis.WALLET, '💰')} <b>Amount Added:</b> <b>+{config.CURRENCY_SYMBOL}{deposit.amount:.2f}</b>\n"
                f"{ce(CustomEmojis.CARD, '💳')} <b>New Wallet Balance:</b> <b>{config.CURRENCY_SYMBOL}{user.balance:.2f}</b>\n\n"
            ...
            try:
                ...
                logger.error(f"Failed to notify user of Cashfree deposit: {e}")

            admin_dep_alert = (
                f"{ce(CustomEmojis.FIRE, '🔔')} <b>AUTO-DEPOSIT CAPTURED VIA CASHFREE!</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"{ce(CustomEmojis.ORDERS, '🧾')} <b>Deposit ID:</b> #{deposit.id}\n"
                f"{ce(CustomEmojis.VERIFIED, '👤')} <b>User:</b> {user.full_name} (@{user.username or 'NoUser'})\n"
                f"{ce(CustomEmojis.WALLET, '💰')} <b>Amount:</b> <b>+{config.CURRENCY_SYMBOL}{deposit.amount:.2f}</b>\n"
                f"{ce(CustomEmojis.CARD, '💳')} <b>Gateway:</b> Cashfree (CF Pay ID: {payment_id})"
            )
            for admin_id in config.ADMIN_IDS:
                try:
                    await bot.send_message(admin_id, admin_dep_alert)
...
def create_webhook_app(bot: Bot, dp=None) -> web.Application:
    ...
    app.router.add_get("/webhook/cashfree", handle_cashfree_webhook_get)
    app.router.add_post("/webhook/cashfree", handle_cashfree_webhook)
    app.router.add_get("/webhook/gateway", handle_cashfree_webhook_get)
    app.router.add_post("/webhook/gateway", handle_cashfree_webhook)
    return app
